...
function render(){
 ...
 $("todayRevenue").textContent=money(sales.reduce((a,s)=>a+s.total,0));$("todayOrders").textContent=sales.length;
...
document.querySelectorAll("nav button").forEach(b=>b.onclick=()=>{document.querySelectorAll("nav button").forEach(x=>x.classList.toggle("active",x===b));document.querySelectorAll(".screen").forEach(s=>s.classList.toggle("active",s.id===b.dataset.screen))});
